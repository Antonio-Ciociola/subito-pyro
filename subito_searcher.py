    ...
    try:
        for search in queries.items():
            for url in search[1].items():
                for minP in url[1].items():
                    for maxP in minP[1].items():
                        print(url[0], search[0], notify, minP[0], maxP[0])
                        await run_query(url[0], search[0], notify, minP[0], maxP[0])
...
async def run_query(url, name, notify, minPrice, maxPrice):
    ...
    if len(msgs) > 0:
        if notify:
            ...
            print("\n".join(msgs))
            print("\n{} new elements have been found.".format(len(msgs)))
        save_queries()
    else:
